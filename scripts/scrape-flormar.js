...
function extractImages(product) {
  ...
  // Extract CDN URLs from the full JSON blob
...
function extractVariants(product) {
  const rawVars =
    product.variants || product.options || product.shades ||
    product.colors || product.attributes || [];

  if (!Array.isArray(rawVars) || rawVars.length === 0) {
    return [{
      shade_name: "Standard",
      sku: product.sku || (product.id != null ? String(product.id) : null),
      color_hex: null,
  ...
  return rawVars.map((v, i) => {
    if (typeof v === "string") {
      return { shade_name: v, sku: null, color_hex: null, swatch_image_url: null, stock_qty: 99 };
    }
    const shadeName =
      v.name || v.shade_name || v.title || v.value || v.label || v.option || `Shade ${i + 1}`;
    ...
    return {
      shade_name: String(shadeName).trim(),
      sku: sku || null,
...
async function fetchAllProducts(categoryId, categorySlug) {
  ...
  while (true) {
    ...
    for (const raw of items) {
      const p = parseProduct(raw, categorySlug);
      if (p) products.push(p);
    }

    console.log(`    page ${page}: ${items.length} raw items → ${products.length} parsed`);
  ...
  try {
    ...
    categories = raw
      .map(c => ({
        ...
        slug: c.slug || c.handle || slugify(c.name || c.title || ""),
      }))
    ...
    console.error("  If running locally, make sure flormar.tn is reachable from your machine.");
    process.exit(1);
  }
