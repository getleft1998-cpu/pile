 ...
 *   - Replaces variants per product
 *   - Downloads images from cdn.converty.shop → uploads to product-images bucket
 *   - Writes IMPORT_REPORT.md
 *
...
const REPORT_FILE = path.join(process.cwd(), "IMPORT_REPORT.md");
...
function writeReport(status, reason, totals, result) {
  ...
  if (totals) {
    lines.push("", "## Scrape Totals", "", "| Metric | Count |", "|--------|------:|",
      ...
      `| Missing prices | ${totals.missingPrices} |`,
...
async function main() {
  ...
  console.log(`  Image upload: ${SKIP_UPLOAD ? "SKIPPED (set SKIP_IMAGE_UPLOAD=true)" : "enabled"}\n`);

  if (!products.length) {
    writeReport("skipped", "0 products in dry-run.json", totals, null);
    process.exit(0);
  }
  ...
  const result = {
    catsInserted: 0, productsInserted: 0, variantsInserted: 0,
    imagesUploaded: 0, imagesFailed: 0, productsFailed: 0,
    errors: [],
  };
  ...
  for (const product of products) {
    try {
      ...
      const imageRecords = [];
      const imageSrcs = (product.images || []).slice(0, 5);

      for (let i = 0; i < imageSrcs.length; i++) {
        let finalUrl = imageSrcs[i];

        if (!SKIP_UPLOAD) {
          const dl = await downloadImage(imageSrcs[i]);
          if (dl) {
            const ext = extFromCt(dl.contentType);
            const filePath = `products/${product.slug}/${Date.now()}-${i}.${ext}`;
            const { data: up, error: upErr } = await supabase.storage
              .from(BUCKET)
              .upload(filePath, dl.data, { contentType: dl.contentType, upsert: true });
            if (up && !upErr) {
              finalUrl = supabase.storage.from(BUCKET).getPublicUrl(up.path).data.publicUrl;
      ...
      result.errors.push(`${product.slug}: ${err}`);
      result.productsFailed++;
    }
  }
  ...
  writeReport("success", null, totals, result);
}

main().catch(err => {
  console.error("Fatal:", err);
  writeReport("failed", String(err), null, null);
  process.exit(1);
});
