...
export async function POST(req: NextRequest) {
  // Auth: cookie OR X-Admin-Token header OR ?token= query param
  ...
  const { error: updErr } = await supabase.storage.updateBucket("product-images", { public: true });
  if (updErr) await supabase.storage.createBucket("product-images", { public: true });
